...
export class CommandHandler {
  ...
  async handleCommand() {
    try {
      ...
      if (banUser?.status && !this.config.ADMIN_IDS.includes(event.senderID)) {
        return api.sendMessage(`You have been banned for the reason: ${banUser.reason}`, threadID);
      }
      ...
      if (isGroup) {
        ...
        if (banThread?.status && !this.config.ADMIN_IDS.includes(event.senderID)) {
          return api.sendMessage(`The group has been banned for the reason: ${banThread.reason}`, threadID);
        }
      }
      ...
      if (!command) {
        return api.sendMessage("The command does not exist.", threadID, messageID);
      }
      ...
      if (!this.config.ADMIN_IDS.includes(senderID)) {
        if (timeStamps.has(senderID)) {
          ...
          if (currentTime < expTime) {
            ...
            return api.sendMessage(`Please wait... ${timeLeft.toFixed(1)} seconds to use ${prefix}${command.name}`, threadID, messageID);
          }
        }
  ...
  async handleReply() {
    ...
    if (!command) {
      return await this.arguments.api.sendMessage("Missing data to process feedback!", this.arguments.event.threadID, this.arguments.event.messageID);
    }
    ...
    if (parseInt(reply.expires)) {
      setTimeout(() => {
        ...
        log([
          ...
          { message: `Removed response data for the command ${reply.name} After ${reply.expires} Second <${messageReply.messageID}>`, color: "green" },
        ]);
      }, reply.expires * 1000);
    }
  ...
  async handleReaction() {
    ...
    if (!command) {
      return await this.arguments.api.sendMessage("Missing data to process feedback", this.arguments.event.threadID, messageID);
    }
